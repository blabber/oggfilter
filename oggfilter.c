...
int		main       (int, char **);
int		check_file (char *, filter);
double		option_parse_double(char *);
...
check_file(char *filename, filter filter)
{
	...
	if (ov_fopen(filename, &ovf) != 0) {
		...
		if (filter.min_length_flag) {
			if (filter.min_length >= time)
				return 0;
		}
		if (filter.max_length_flag) {
			if (filter.max_length <= time)
				return 0;
		}
		ov_clear(&ovf);
	}
